...
Обновление `2.5.3` устанавливается непосредственно на любую систему ветки
`2.5.x`; промежуточные патчи не требуются. Скачайте архив и соответствующий
checksum из Release `v2.5.0`, поместите оба файла в `/opt/cdr/update`, затем
откройте `Система -> Обновления` и примените пакет:

```bash
cd /opt/cdr/update
sha256sum -c update-2.5.3.tar.gz.sha256
```

Перед изменением updater создаёт Rollback согласно манифесту пакета. После
проверки системы нажмите `Commit`; при проблеме до Commit доступен `Rollback`.
